			...
			if (selector.uuids.length > 0) {
				...
				if (!localASSet.content.isEqual(remoteASSet.content)) {
					...
					if (
						...
						const patch = localASSet.content.diff(remoteASSet.content);

						if (patch.added.length > 0 || patch.removed.length > 0) {
							const patchedASSetContent = localASSet.content.patch(patch);

							localASSet.content = patchedASSetContent;

							this.IRRManager.replace(selector.uuids[0], localASSet);
						}
					}
				}
