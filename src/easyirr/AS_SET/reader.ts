...
export function parseMember(
	...
	const finishMemberWithDetails = (
		...
		if (baseMember instanceof AS_SET_Member) {
			...
			if ("flatten" in baseOptions) baseMember.flatten = baseOptions.flatten;
			...
			if ("irrdGraphQLEndpoint" in baseOptions)
				baseMember.irrdGraphQLEndpoint = baseOptions.irrdGraphQLEndpoint;
		...
		throw new Error(`Unsupported member type: ${baseMember.constructor.name}`);
	};
	...
	if (Object.keys(member).length === 1) {
		...
		const memberValue:
			| easy_as_set.contentConf.ASNMemberOptions
			| easy_as_set.contentConf.asSetMemberOptions = (member[key] as any) ?? {};
	...
	if ("type" in member && "value" in member) {
		...
		switch (type) {
			case "ASN":
				if (typeof value === "number") baseMember = new ASN_Member(value);
				if (typeof value === "string" && value.startsWith("AS")) {
					const asn = parseInt(value.slice(2));
					if (isNaN(asn)) throw new Error(`Invalid AS_NUMBER value: ${value}`);
